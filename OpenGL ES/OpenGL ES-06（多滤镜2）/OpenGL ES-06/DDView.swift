...
/*
 ...
 单个滤镜：直接用一个片元着色器直接使用渲染的framebuffer就行（imgTexture -> framebuffer）
         使用两个片元着色器(滤镜片元着色器 + 渲染片元着色器): （imgTexture -> tempFrameBuffer -> framebuffer）
         
                                                      GL_TEXTURE1   (tempTexture, GL_TEXTURE0)
 多个滤镜：使用两个片元着色器(滤镜片元着色器 + 渲染片元着色器)：（imgTexture -----> tempFrameBuffer -----> framebuffer）
         使用三个片元着色器(滤镜片元着色器1 + 滤镜片元着色器2 + 渲染片元着色器)：（imgTexture -> tempFrameBuffer1 -> tempFrameBuffer2 -> framebuffer）
         ⚠️：不管多少个片元着色器，都要按照顺序来： 原始纹理 -> 片元着色器1 -> ... -> framebuffer, 不然效果就会一直叠加‼️
...
class DDView: UIView {
    ...
    private var temperatureTexture: GLuint = 0
    private var saturationTexture: GLuint = 0
    private var temperatureTexture1: GLuint = 0
    private var saturationTexture1: GLuint = 0
    
    private var temperatureFrameBuffer: GLuint = 0
    private var saturationFrameBuffer: GLuint = 0
    private var temperatureFrameBuffer1: GLuint = 0
    private var saturationFrameBuffer1: GLuint = 0
    ...
    var hasTemperature: Bool = false //是否处理过色温
    var hasSaturation: Bool = false //是否处理过饱和度
    
    var outImage: UIImage?
    var outImageRight: UIImage?
    ...
    var temperature: CGFloat = 0 {
        didSet {
            ...
            //绑定frameBuffer
            glBindFramebuffer(GLenum(GL_FRAMEBUFFER), temperatureFrameBuffer)
            //设置清屏颜色
            ...
            let texture = (hasSaturation) ? saturationTexture : imgTexture
            glBindTexture(GLenum(GL_TEXTURE_2D), texture)
            glUniform1i(glGetUniformLocation(temperatureProgram, "inputImageTexture"), 1) //⚠️：idx == GL_TEXTURE1
            ...
            //✌️第二步：绘制第一个滤镜
            // 绘制纹理完毕，开始绘制到屏幕上
            self.render(temperatureTexture)
    ...
    var saturation: CGFloat = 0 {
        didSet {
            ...
            //绑定frameBuffer
            glBindFramebuffer(GLenum(GL_FRAMEBUFFER), saturationFrameBuffer)
            //设置清屏颜色
            glClearColor(0.0, 1.0, 0.0, 1.0)
            //清除屏幕
            glClear(GLbitfield(GL_COLOR_BUFFER_BIT))
            ...
            // 传递原始纹理数据
            glActiveTexture(GLenum(GL_TEXTURE1))
            let texture = (hasTemperature) ? temperatureTexture : imgTexture
            glBindTexture(GLenum(GL_TEXTURE_2D), texture)
            glUniform1i(glGetUniformLocation(saturationProgram, "inputImageTexture"), 1) //⚠️：idx == GL_TEXTURE1
...
//        assert(self.checkFramebuffer(error: &error), error?.userInfo["ErrorMessage"] as! String)
        ...
        self.setupSaturationTexture()
        self.setupTemperature1()
        self.setupSaturationTexture1()
        
        //10.设置纹理图片
        self.setupTextureWithImage(image)
        
        //11.渲染
        self.render(imgTexture)
...
//            }
    ...
    //渲染 处理shaders编译
    private func compileRenderShaders() {
        //1. 创建program
        let program: GLuint = glCreateProgram()
        
        //2. 编译顶点着色器程序、片元着色器程序
        let vertShader = compileShader(shaderName: "shaderv.vsh", shaderType: GLenum(GL_VERTEX_SHADER))
        let fragShader = compileShader(shaderName: "fragmentShader.fsh", shaderType: GLenum(GL_FRAGMENT_SHADER))
        ...
        var linkStatus: GLint = 0
        //获取链接状态
        glGetProgramiv(renderProgram, GLenum(GL_LINK_STATUS), &linkStatus)
        if linkStatus == GL_FALSE {
            NSLog("link error")
            let message = UnsafeMutablePointer<GLchar>.allocate(capacity: 512)
            glGetProgramInfoLog(renderProgram, GLsizei(MemoryLayout<GLchar>.size * 512), nil, message)
            let str = String(utf8String: message)
            print("error = \(str ?? "没获取到错误信息")")
            return
        }
        
        NSLog("renderProgram link success!")
    }
...
//        CGContextRelease(spriteContext);
    ...
    private func setupTemperature() {
        ...
        //绑定纹理
        glBindTexture(GLenum(GL_TEXTURE_2D), temperatureTexture)
        //将图片载入纹理
        /*
        ...
        /*
         ...
         5.level,一般为0
         */
        glFramebufferTexture2D(GLenum(GL_FRAMEBUFFER), GLenum(GL_COLOR_ATTACHMENT0), GLenum(GL_TEXTURE_2D), temperatureTexture, 0)
    }
    
    ///10.设置纹理
    private func setupSaturationTexture() {
        //绑定纹理之前,激活纹理
        glActiveTexture(GLenum(GL_TEXTURE1))
        //申请纹理标记
        glGenTextures(1, &saturationTexture)
        //绑定纹理
        glBindTexture(GLenum(GL_TEXTURE_2D), saturationTexture)
        //将图片载入纹理
        /*
         glTexImage2D (GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const GLvoid *pixels)
         参数列表:
        ...
        /*
         ...
         5.level,一般为0
         */
        glFramebufferTexture2D(GLenum(GL_FRAMEBUFFER), GLenum(GL_COLOR_ATTACHMENT0), GLenum(GL_TEXTURE_2D), saturationTexture, 0)
    }
    ...
    private func render(_ texture: GLuint) {
        //使用渲染着色器
...
#warning("注意⚠️：想要获取shader里面的变量，这里要记住要在glLinkProgram后面、后面、后面")
        ...
        glEnableVertexAttribArray(GLuint(glGetAttribLocation(renderProgram, "position")))
        ...
        glVertexAttribPointer(GLuint(glGetAttribLocation(renderProgram, "position")), 3, GLenum(GL_FLOAT), GLboolean(GL_FALSE), GLsizei(MemoryLayout<GLfloat>.size * 5), UnsafeRawPointer(bitPattern: MemoryLayout<GLfloat>.size * 0))
        ...
        //设置合适的格式从buffer里面读取数据
        glEnableVertexAttribArray(GLuint(glGetAttribLocation(renderProgram, "inputTextureCoordinate")))
        ...
        //参数5：stride,连续顶点属性之间的偏移量，默认为0；
        //参数6：指定一个指针，指向数组中的第一个顶点属性的第一个组件。默认为0
        glVertexAttribPointer(GLuint(glGetAttribLocation(renderProgram, "inputTextureCoordinate")), 2, GLenum(GL_FLOAT), GLboolean(GL_FALSE), GLsizei(MemoryLayout<GLfloat>.size * 5), UnsafeRawPointer(bitPattern: MemoryLayout<GLfloat>.size * 3))

        //纹理
        glActiveTexture(GLenum(GL_TEXTURE1))
        glBindTexture(GLenum(GL_TEXTURE_2D), texture)
        glUniform1i(glGetUniformLocation(renderProgram, "inputImageTexture"), 1) //⚠️：idx == GL_TEXTURE1
//
//        //饱和度
//        glUniform1f(glGetUniformLocation(saturationProgram, "saturation"), GLfloat(saturation))
    ...
    //MARK: - ⚠️：不管多少个片元着色器，都要按照顺序来： 原始纹理 -> 片元着色器1 -> ... -> framebuffer, 不然效果就会一直叠加‼️
    ///10.设置纹理
    private func setupTemperature1() {
        //绑定纹理之前,激活纹理
        glActiveTexture(GLenum(GL_TEXTURE0))
        //申请纹理标记
        glGenTextures(1, &temperatureTexture1)
        //绑定纹理
        glBindTexture(GLenum(GL_TEXTURE_2D), temperatureTexture1)
        //将图片载入纹理
        /*
         glTexImage2D (GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const GLvoid *pixels)
         参数列表:
        ...
        //申请_tempFramesBuffe标记
        glGenFramebuffers(1, &temperatureFrameBuffer1)
        //绑定FrameBuffer
        glBindFramebuffer(GLenum(GL_FRAMEBUFFER), temperatureFrameBuffer1)
        ...
        /*
         ...
         5.level,一般为0
         */
        glFramebufferTexture2D(GLenum(GL_FRAMEBUFFER), GLenum(GL_COLOR_ATTACHMENT0), GLenum(GL_TEXTURE_2D), temperatureTexture1, 0)
    }
    
    ///10.设置纹理
    private func setupSaturationTexture1() {
        //绑定纹理之前,激活纹理
        glActiveTexture(GLenum(GL_TEXTURE0))
        //申请纹理标记
        glGenTextures(1, &saturationTexture1)
        //绑定纹理
        glBindTexture(GLenum(GL_TEXTURE_2D), saturationTexture1)
        //将图片载入纹理
        /*
         glTexImage2D (GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const GLvoid *pixels)
         参数列表:
        ...
        //申请_tempFramesBuffe标记
        glGenFramebuffers(1, &saturationFrameBuffer1)
        //绑定FrameBuffer
        glBindFramebuffer(GLenum(GL_FRAMEBUFFER), saturationFrameBuffer1)
        ...
        /*
         ...
         5.level,一般为0
         */
        glFramebufferTexture2D(GLenum(GL_FRAMEBUFFER), GLenum(GL_COLOR_ATTACHMENT0), GLenum(GL_TEXTURE_2D), saturationTexture1, 0)
    }
    
    func renderInSequence() {
        // ------------------------- 1 -----------------------------
        //☝️第一步：绘制第一个滤镜
        //使用色温着色器
        glUseProgram(temperatureProgram)
        ...
        glActiveTexture(GLenum(GL_TEXTURE0))
        glBindTexture(GLenum(GL_TEXTURE_2D), imgTexture)
        glUniform1i(glGetUniformLocation(temperatureProgram, "inputImageTexture"), 0) //⚠️：idx == GL_TEXTURE1
        ...
        glActiveTexture(GLenum(GL_TEXTURE0))
        glBindTexture(GLenum(GL_TEXTURE_2D), temperatureTexture1)
        glUniform1i(glGetUniformLocation(saturationProgram, "inputImageTexture"), 0) //⚠️：idx == GL_TEXTURE1
        ...
        outImageRight = getImageFromBuffer(width: Int(self.frame.size.width * self.contentScaleFactor), height: Int(self.frame.size.height * self.contentScaleFactor))
    }
