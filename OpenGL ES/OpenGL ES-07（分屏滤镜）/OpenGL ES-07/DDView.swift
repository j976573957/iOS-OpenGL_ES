...
#warning("法二")
    ...
    //6.设置VBO (Vertex Buffer Objects)
    func setupVBO() {
        let image = UIImage(named: "timg-3")!
        let realRect = AVMakeRect(aspectRatio: image.size, insideRect: self.bounds)
        let widthRatio: GLfloat = GLfloat(realRect.size.width / self.bounds.size.width)
        let heightRatio: GLfloat = GLfloat(realRect.size.height / self.bounds.size.height)
