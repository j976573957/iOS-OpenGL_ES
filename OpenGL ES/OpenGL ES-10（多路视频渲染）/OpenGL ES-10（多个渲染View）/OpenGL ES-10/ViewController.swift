...
class ViewController: UIViewController, AVPlayerItemOutputPullDelegate {
    ...
    @IBOutlet var renderView0: DDView!
    @IBOutlet var renderView1: DDView!
    @IBOutlet var renderView2: DDView!
    @IBOutlet var renderView3: DDView!
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        NSLog("maxTextureSize---> \(maxTextureSize)")
    }
    ...
    @objc func displayLinkDidUpdate(_ sender: CADisplayLink) {
        ...
        if let sampleBuffer = self.reader.readBuffer() {
            ...
            self.renderView0.renderBuffer(pixelBuffer: pixelBuffer!, position: 1)
        }
        ...
        if let sampleBuffer = self.reader2.readBuffer() {
            let pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer)
            self.renderView2.renderBuffer(pixelBuffer: pixelBuffer!, position: 1)
        }
//
//        if let sampleBuffer = self.reader3.readBuffer() {
//            let pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer)
//            self.renderView3.renderBuffer(pixelBuffer: pixelBuffer!, position: 2)
//        }
    }
