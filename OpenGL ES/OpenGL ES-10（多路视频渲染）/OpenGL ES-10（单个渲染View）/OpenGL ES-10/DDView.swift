...
class DDView: UIView {
    ...
    var texture1: CVOpenGLESTexture?
    var texture2: CVOpenGLESTexture?
    var texture3: CVOpenGLESTexture?
    var texture4: CVOpenGLESTexture?
    ...
    override func layoutSubviews() {
        ...
        let cacheResult1: CVReturn = CVOpenGLESTextureCacheCreate(kCFAllocatorDefault, nil, self.myContext, nil, &textureCache)
        if (cacheResult1 != kCVReturnSuccess) {
            NSLog("CVOpenGLESTextureCacheCreate fail1 %d", cacheResult1)
        }
...
#warning("法二")
    ...
    //设置纹理
    func renderBuffer(pixelBuffer: CVPixelBuffer?, position: Int) {
        if let pixelBuffer = pixelBuffer {//注意⚠️：释放内存，要不然会卡住
            if texture1 != nil { texture1 = nil }
            if texture2 != nil { texture2 = nil }
            if texture3 != nil { texture3 = nil }
            if texture4 != nil { texture4 = nil }
            ...
            case 1:
                ...
                glBindTexture(CVOpenGLESTextureGetTarget(texture1!), CVOpenGLESTextureGetName(texture1!))
            ...
            case 2:
                ...
                let ret: CVReturn = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault,
                                                                                 ...
                                                                                 &texture2);
                if ((ret) != 0) {
                    NSLog("CVOpenGLESTextureCacheCreateTextureFromImage ret1: %d", ret)
                    return
                }
                glBindTexture(CVOpenGLESTextureGetTarget(texture2!), CVOpenGLESTextureGetName(texture2!))
                glTexParameteri(GLenum(GL_TEXTURE_2D), GLenum(GL_TEXTURE_MAG_FILTER), GL_LINEAR)
                glTexParameteri(GLenum(GL_TEXTURE_2D), GLenum(GL_TEXTURE_MIN_FILTER), GL_LINEAR)
                glTexParameteri(GLenum(GL_TEXTURE_2D), GLenum(GL_TEXTURE_WRAP_S), GL_CLAMP_TO_EDGE)
                glTexParameteri(GLenum(GL_TEXTURE_2D), GLenum(GL_TEXTURE_WRAP_T), GL_CLAMP_TO_EDGE)
            ...
            case 3:
                ...
                let ret: CVReturn = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault,
                                                                                 ...
                                                                                 &texture3);
                if ((ret) != 0) {
                    NSLog("CVOpenGLESTextureCacheCreateTextureFromImage ret2: %d", ret)
                    return
                }
                glBindTexture(CVOpenGLESTextureGetTarget(texture3!), CVOpenGLESTextureGetName(texture3!))
                glTexParameteri(GLenum(GL_TEXTURE_2D), GLenum(GL_TEXTURE_MAG_FILTER), GL_LINEAR)
                glTexParameteri(GLenum(GL_TEXTURE_2D), GLenum(GL_TEXTURE_MIN_FILTER), GL_LINEAR)
                glTexParameteri(GLenum(GL_TEXTURE_2D), GLenum(GL_TEXTURE_WRAP_S), GL_CLAMP_TO_EDGE)
                glTexParameteri(GLenum(GL_TEXTURE_2D), GLenum(GL_TEXTURE_WRAP_T), GL_CLAMP_TO_EDGE)
            ...
            case 4:
                ...
                let ret: CVReturn = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault,
                                                                                 ...
                                                                                 &texture3);
                if ((ret) != 0) {
                    NSLog("CVOpenGLESTextureCacheCreateTextureFromImage ret3: %d", ret)
                    return
                }
                glBindTexture(CVOpenGLESTextureGetTarget(texture3!), CVOpenGLESTextureGetName(texture3!))
                glTexParameteri(GLenum(GL_TEXTURE_2D), GLenum(GL_TEXTURE_MAG_FILTER), GL_LINEAR)
...
#warning("注意⚠️：想要获取shader里面的变量，这里要记住要在glLinkProgram后面、后面、后面")
        ...
        //单个纹理可以不用设置
        glUniform1i(glGetUniformLocation(self.myPrograme, "texture1"), 0)
        glUniform1i(glGetUniformLocation(self.myPrograme, "texture2"), 1)
        glUniform1i(glGetUniformLocation(self.myPrograme, "texture3"), 2)
        glUniform1i(glGetUniformLocation(self.myPrograme, "texture4"), 3)
